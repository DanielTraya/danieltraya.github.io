 ...
 **/
define('MERCHANT_ID', 'TESTDANIEL');
define('MERCHANT_PASSWORD', 'WDb72jknFJhfw4J');
...
?>
  ...
  if (isset($_POST['submit'])) {
    ...
    if (!is_numeric($parameters['refno'])) {
      $errors[] = 'Reference No. should be a number.';
    }
    else if ($parameters['refno'] <= 0) {
      $errors[] = 'Reference No. should be greater than 0.';
    }
    ...
    if (empty($errors)) {
      ...
      if ($environment == ENV_TEST) {
        $url = 'http://test.dragonpay.ph/Pay.aspx?&procid=BDO';
      }
